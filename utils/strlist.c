...
char* get_str_in_list(struct strlist* head, char* str)
{
    char* rtstr;
    while((head = get_next_item(head)) != NULL)
    {
        if(strcmp(str, head->str) == 0)
        {
            return str;
...
void test_strlist(void)
{
    ...
    insert_front(list, "agustin");
    insert_front(list, "peep");
    insert_front(list, "ana");
    print_list(list);
    p = list;
    printf("Print all list with get_next_str:\n");
